    ...
    static class Compiler
    {
        ...
        public static void Decompile(string romPath, string codePath)
        {
            ...
            using (StreamWriter writer = new StreamWriter(codePath))
            {
                ...
                //Flag used to signal if code is possibly unreachable, for example if it appears after an unknown opcode
                bool possibleUnreachableCode = false;
                ...
                while (reader.Read(currentInstruction, 0, 2) != 0)
                {
                    ...
                    switch (opcode & 0xF000)
                    {
                        ...
                        case 0xE000:
                            switch (byte2)
                            {
                                ...
                                case 0xA1:
                                    currentLineText += "Skip next instruction if key at " + instructionString[1] + " is not pressed";
                                    break;
                                default:
                                    unknownOpcode = true;
                                    break;
                    ...
                    if (unknownOpcode)
                    {
                        ...
                        possibleUnreachableCode = true;
                    }
                    else if (possibleUnreachableCode)
                    {
                        currentLineText += ", Possibly unreachable";
                    }
